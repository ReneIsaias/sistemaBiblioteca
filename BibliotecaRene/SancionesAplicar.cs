...
{
    public partial class SancionesAplicar : Form
    {
        Biblioteca obj = new Biblioteca();
        public string user;
        public SancionesAplicar()
        {
            InitializeComponent();
        }

        private void SancionesAplicar_Load(object sender, EventArgs e)
        {
            obj.conectaRene();
            obj.llenaCuadriculaRene(dataGridView1, "EXEC llenaSanciones '" + 1 + "'");
            validarUsuario();
        }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    obj.ejecutaRene("EXEC sancionesInUp '" + label7.Text + "','" + textBox1.Text + "','" + textBox2.Text + "','" + comboBox2.SelectedIndex + "','" + label8.Text + "'");
                    if (obj.leerRene.Read())
                    {
                        MessageBox.Show(obj.leerRene[0].ToString());
                    }
                    //MessageBox.Show("Categoria Guardada Correctamente");
                    SancionesAplicar_Load(sender, e);
        ...
        {
            obj.llenaCuadriculaRene(dataGridView1, "EXEC llenaSancionesDetalle '" + 1 + "'");
        }
        ...
        {
            obj.llenaCuadriculaRene(dataGridView1, "EXEC buscaSanciones '" + textBox3.Text +"'");
            obj.leerRene.Close();
        }
    }
}
